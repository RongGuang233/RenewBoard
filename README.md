...
下图为 1.2.2 模拟器中的演示数据；首次安装默认为空账本。

<img src="docs/images/date-picker-v1.2.2.png" width="250" alt="独立的年份与月份入口"> <img src="docs/images/date-months-v1.2.2.png" width="250" alt="直接点选月份"> <img src="docs/images/device-category-picker-v1.2.2.png" width="250" alt="可搜索的设备分类图标网格"> <img src="docs/images/device-sold-detail-v1.2.2.png" width="250" alt="简洁的设备详情">

## 可以做什么

- 62 项常见会员与自定义分类快捷选择，支持搜索；官方应用商店/官网图标，无需网络加载。
- 新增、编辑、归档、删除订阅；删除后真实付款仍保留在账本。
- 日期选择有独立的年份、月份入口，月份可直接点选；保留手动输入日期。
- 周、月、年及整数倍周期；自动续费与一次性 / 手动续费。
- 原始扣款日期作为周期锚点，月末与闰年不逐月漂移。默认权益随扣款日期和周期联动，手动修改后独立计算，可恢复跟随。
- 话费余额账户：当前余额、固定月费与扣费日推算剩余余额和充值提醒；月费计入未来30天预计扣款，打开应用时按设定扣费日补记余额查询日期之后的月费支出；充值保留记录、只增加余额、不重复计支出。支持余额校准，转换已有话费订阅时保留历史付款。
...
- 明细支持搜索、订阅付款/话费扣费/话费充值/已删订阅筛选、单笔及批量删除；删除订阅时可选择是否同时删除付款。仅删除付款不会撤销权益或改变话费余额。
...
- 设备列表默认服役中，通过下拉查看已退役、已卖出和待购买；记录金额、服役日期和备注，退役记录退役日期，卖出记录卖出日期与金额，查看服役天数及日均购入成本。设备金额单独管理，不自动加入订阅支出。
...
普通订阅的预计扣款不会自动变为付款记录，请实际扣款后选择“记录付款 / 提前续费”。话费采用按月记账：按已设置月费补记，金额并非从运营商获取；余额查询日期之前的历史不倒推，修改月费不重算已记账记录。充值在明细中单列、不计支出。没有支付处理、多设备合并或网页同步。
