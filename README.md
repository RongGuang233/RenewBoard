...
下图为 1.8.0 模拟器中的虚构演示数据；首次安装默认为空账本。

<img src="docs/images/recharge-v1.8.0.png" width="250" alt="独立充值页与充值后余额预览"> <img src="docs/images/trend-range-v1.8.0.png" width="250" alt="完整七档趋势范围选择"> <img src="docs/images/device-purchase-v1.8.0.png" width="250" alt="确认实际购入金额与服役日期">

## 可以做什么

- 62 项常见会员与自定义分类快捷选择，支持搜索和最近使用快捷选择；官方应用商店/官网图标，无需网络加载。
- 新增、编辑、归档、删除订阅；列表支持搜索、自动续费筛选与排序，已选续费筛选显示可清除标签，排序方式直接可见；从详情返回保留查询和滚动位置；归档详情明确停止状态，以恢复使用为主操作。过期项目默认收起。删除后真实付款仍保留在账本。
...
- 账本以统计概览为首页，显示净支出，可展开查看付款与退款构成；从完整菜单选择七档支出趋势（近30天/3个月/6个月/12个月/按年/5年/全部）、应用支出排行和付款明细。点击趋势柱仅更新应用排行，明细入口跟随所选时段；点击应用排行可查看该应用在所选时段的付款。明细为独立页面，按日期分组。未补录的外币金额单独提示。
...
- 设备列表默认服役中，通过下拉查看已退役、已卖出和待购买；列表金额明确区分购入金额、已卖出净花费与待购买预算；记录金额、服役日期和备注，退役记录退役日期，卖出记录卖出日期与金额；停止服役早于卖出时展开独立日期，服役时长按停止服役日期冻结。查看服役天数及日均购入成本，已卖出突出扣除卖价后的净花费。支持搜索名称、类别；排序随状态使用服役/退役/卖出/计划日期，以及购入金额、净花费或预算；待购买隐藏服役时长排序；通过“记为已购买”确认实际金额与开始服役日期，保存后转为服役中，保留原设备资料。设备金额单独管理，不自动加入订阅支出。
- 设置首页按功能分组；到期提醒、本地备份、坚果云 WebDAV、更新与关于分别进入独立页面。WebDAV 配置后优先显示备份状态、立即备份与云端备份，账号和应用密码收进连接设置。
...
- JSON 文件导出 / 恢复；1.8.0可读取旧备份，包含退款和独立卖出日期的新备份请在1.5.0及更新版本恢复。未完成草稿仅保留本机，不包含在账本备份中。坚果云 WebDAV 自动与手动备份。恢复先预览时间、数量，再确认整体替换。
