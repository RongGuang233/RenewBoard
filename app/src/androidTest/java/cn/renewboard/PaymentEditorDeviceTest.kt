...
class PaymentEditorDeviceTest {
    ...
    @Test fun draftSurvivesRecreateAndExitStaysScopedToPlanAndCanBeDiscarded() {
        ...
        compose.onNode(hasText("本次实付总额 ¥") and hasSetTextAction()).assertTextContains("35.80")
        back();compose.onNodeWithText("保留草稿",substring=false).performClick()
        back()
        open(second.name)
        compose.onNode(hasText("本次实付总额 ¥") and hasSetTextAction()).assertTextContains("20")
        back();back()
        open(first.name)
    ...
    @Test fun multiPeriodRenewalRecordsOnePaymentAndClearsDraft() {
        ...
        click("本次续费 1个月")
        fill("续费期数","2")
        compose.onNode(hasText("本次实付总额 ¥") and hasSetTextAction()).assertTextContains("40")
        compose.onNodeWithText("本次续费 2个月").assertExists()
        click("续费权益 2/2 项")
    ...
    @Test fun customTotalSurvivesPeriodChangesAndRecreation() {
        runBlocking {app.repository.update {it.copy(plans=it.plans.map {p->if(p.id==first.id) p.copy(interval=3) else p})}}
        compose.waitForIdle()
        open(first.name)
        click("本次续费 3个月")
        fill("续费期数","2")
        ...
        fill("续费期数","3")
        compose.onNode(hasText("本次实付总额 ¥") and hasSetTextAction()).assertTextContains("35.80")
        compose.onNodeWithText("本次续费 9个月").assertExists()
        compose.waitForIdle()
        compose.activityRule.scenario.recreate()
        compose.waitUntil(10000) {compose.onAllNodes(hasText("本次实付总额 ¥") and hasSetTextAction()).fetchSemanticsNodes().isNotEmpty()}
        compose.onNode(hasText("本次实付总额 ¥") and hasSetTextAction()).assertTextContains("35.80")
        click("本次续费 9个月");fill("续费期数","2")
        compose.onNode(hasText("本次实付总额 ¥") and hasSetTextAction()).assertTextContains("35.80")
        back();compose.onNodeWithText("放弃修改",substring=false).performClick()
        assertTrue(runBlocking {app.repository.read()}.payments.isEmpty())
    }
