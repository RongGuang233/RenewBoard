...
@Dao interface BookDao {
    ...
    @Query("SELECT event FROM ReminderRow") suspend fun reminderEvents(): List<String>
...
class Repository(val db: BookDatabase,
                 private val ledgerChanged: suspend (Ledger, Ledger) -> Unit = { _, _ -> },
    ...
    suspend fun update(transform: (Ledger) -> Ledger) {
        val (before, after) = db.withTransaction {
            val before = read()
            val next = transform(before); Book.validate(next)
    ...
    suspend fun recordMonthlyFees(today:java.time.LocalDate=java.time.LocalDate.now()) {
        val change=db.withTransaction {
            ...
            if(next==old) null else {
                Book.validate(next)
                db.book().put(BookRow(payload=Book.json.encodeToString(next)));old to next
            }
        }
        if(change != null) {
            ledgerChanged(change.first, change.second)
            changed()
        }
    }
...
class RenewApp: Application() {
    ...
    override fun onCreate() {
        super.onCreate()
        repository = Repository(Room.databaseBuilder(this, BookDatabase::class.java, "renewboard.db").build(),
            ledgerChanged = { before, after ->
                kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) { Jobs.clearResolvedReminders(this@RenewApp, before, after) }
            }, changed = { Jobs.backup(this) })
        Jobs.schedule(this)
    }
}
